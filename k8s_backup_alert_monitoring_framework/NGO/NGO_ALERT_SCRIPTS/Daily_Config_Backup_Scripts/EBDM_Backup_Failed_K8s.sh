...
if [ ${#FAILED_BACKUPS[@]} -gt 0 ]; then
  ...
  JSON_ENTRY=$(cat <<EOF
{"ts":"$TIMESTAMP","ip":"$IP","hname":"$HOST","kpi":"$KPI_NAME","value":"$CNT","cnt":"$CNT","app_sub_name":"$APP_SUB_NAME","kpivalue1":"Backup failure detected for Kubernetes $KPI_VALUE1. Check backup job logs and last successful backup timestamp."}
