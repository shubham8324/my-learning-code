...
for ns in $NAMESPACES; do
  ...
  for pod in $pods; do
    # Defensive checks in case ownerReferences is missing
    deploy_name=$(kubectl get pod "$pod" -n "$ns" -o jsonpath='{.metadata.ownerReferences[0].name}' 2>/dev/null || echo "unknown-deploy")
    images=$(kubectl get pod "$pod" -n "$ns" -o jsonpath='{range .spec.containers[*]}{.image}{" "}{end}' | sed 's/ $//')

    for image in $images; do
      line="${ns}|${deploy_name}|${image}"
